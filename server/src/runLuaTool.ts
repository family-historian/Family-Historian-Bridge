...
Before writing ANY script, search first — not optional, even for calls that feel familiar. Call search_fh_help (and search_gedcom_knowledge for domain/FTF concepts) for every FH function, item-pointer method, or fhUtils call you haven't already confirmed this conversation, rather than guessing and fixing it against the user's real project. This matters for both reads and writes — before hand-rolling a MoveToFirstRecord/MoveNext walk, or fhCreateItem/fhSetValueAsLink/fhDeleteItem, search_fh_help for whether fhu (already a global in this sandbox — never call require('fhUtils'), it returns nil here) has a purpose-built helper: fhu.records/fhu.allItems for iteration, fhu.createIndi/fhu.createFact/fhu.addFamilyAsChild/addFamilyAsSpouse for mutation — before reaching for low-level primitives. Specifically, before any loop that navigates items using MoveToFirstChildItem/MoveNext or MoveToFirstRecord/MoveNext, search for "fhNewItemPtr iteration" to confirm you're using fhNewItemPtr() (not fhCreateItem()) — using fhCreateItem() for iteration creates unwanted database records and can trigger write-protection errors.
