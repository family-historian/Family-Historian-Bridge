...
Use this when search_fh_help's excerpt doesn't contain enough of the page to answer the question, or when you only know a fragment of what you're looking for — a specific function name, a line from a sample script, an error string — but not which page it lives on. This is the fallback to reach for before web search: the corpus is already local, so a pattern that would find something on the FH help website will usually find it here too. To list every valid bare fh* global name in one call (e.g. before deciding a name is genuinely unknown), grep for the page titled "Function Index" — a single entry covering all of them by signature. fhu.* (the sandboxed fhUtils proxy) has no single-entry index — grep the breadcrumb "fhUtils.md" instead to list every fhu.* entry across a few calls, or grep an exact method name once known.
