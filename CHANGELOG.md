...
- `run_lua` scripts that enumerate the raw GEDCOM tree (`MoveToFirstRecord`, `fhu.records`/`allItems`/`indiList`) are now rejected outright while a Visibility level is restricted -- the tool description and a new `run-lua-guidance-visibility-settings` corpus entry steer toward the filtered `fhBridge.*` helpers instead. Raw `fh*`/`fhu` calls that don't enumerate still aren't filtered by Visibility settings -- a deliberate, documented trade-off (docs/adr/0038), not a gap.
...
- `fhBridge.findByNames` matches now carry a `lifeDates` field (e.g. `"1865-1932"`, issue #138) via FH's own `LifeDates` built-in, computed once per match during the existing scan; key is omitted entirely (not `""`) when FH has nothing to report. `getFamilyGroup`/`getAncestors`/`getDescendants` output is unchanged -- `indiDescriptor` itself wasn't touched.
- `search_fh_help`/`search_gedcom_knowledge`'s token-overlap fallback now tolerates basic morphological variants (issues #139/#140): a lightweight suffix-stripping stemmer lets a query word match a differently-inflected corpus word (`flags`/`flag`, `delete`/`deletion`/`deleted`/`deleting`), compared as whole stemmed words (not a raw substring of the stem, which would false-positive on unrelated words like `date` inside `database`). When a result only matched via this fallback, its excerpt now windows around the matched word instead of the entry's opening chars.
...
- `dtDate` accepts a live Date object (`fhNewDate(...)`), the `{year=, month=, day=[,
  subtype=]}` table shorthand, or a plain string (e.g. `"1901"`) — all resolved via a new
  shared `familyHelper.resolveDate(value, callerName)`, which parses a string through FH's
  own `Date:SetValueAsText(text, false)` and rejects an unrecognized one before any write
  happens (`docs/adr/0030-shared-resolvedate-accepts-date-object-table-or-string.md`).
