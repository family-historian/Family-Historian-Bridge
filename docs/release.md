...
- `bridge/dist/Claude MCP Bridge.fh_lua`'s `@Version:` header is stamped by
  `bridge/scripts/build.lua` from `server/package.json` (issue #89), the same way it
  already stamps `@LastUpdated:` with the build date.

The committed stub `bridge/Claude MCP Bridge.fh_lua` is the build's source, so its
`@Version:` and `@LastUpdated:` headers go stale (the build only stamps the `dist/` copy).
On every release build, set them by hand to the new version and today's date so the
committed stub matches what ships.
