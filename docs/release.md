...
Only **two** copies are hand-maintained:
...
- `bridge/Claude MCP Bridge.fh_lua` — the `@Version:` header field (near the top of the
  file). The last genuinely hand-kept second copy; issue #89 proposes having
  `bridge/scripts/build.lua` inject it from `server/package.json` to close this for good.
...
- **Copied-value drift** (step 4) has bitten this repo repeatedly, in the same shape every
  ...
  guarded by a test that only compared two of them to each other (issue #85). Each was
  fixed the same way — generate or read from one source, then test that the source matches
  reality. **One hand-maintained copy is left**: the Bridge's `@Version:` header (issue
  #89). Treat any new copied value as a defect waiting to happen.
