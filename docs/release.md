...
- `bridge/dist/AI Assistant Connector.fh_lua`'s `@Version:` header is stamped by
  `bridge/scripts/build.lua` from `server/package.json` (issue #89), the same way it
  already stamps `@LastUpdated:` with the build date.

The committed stub `bridge/AI Assistant Connector.fh_lua` is the build's source, so its
...
```
This produces `server/dist/` and `bridge/dist/AI Assistant Connector.fh_lua` — the latter is a
