...
`buildManifest({ version })` function — no file I/O, so it can be unit-tested directly
...
- `installer/build-dxt.mjs` (`node installer/build-dxt.mjs --verify`) is cross-platform
  ...
  deliberately, since nothing about staging a `.mcpb` is platform-specific the way bundling
